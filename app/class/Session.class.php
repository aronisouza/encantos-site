    ...
    */
    ...
    private function setTraffic() {
        ...
        if (!$this->Traffic):
            ...
            $createSiteViews->ExeCreate('adm_siteviews', $ArrSiteViews);
        else:
            if (!$this->getCookie()):
            ...
            $updateSiteViews->ExeUpdate('adm_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function TrafficUpdate() {
        ...
        $updatePageViews->ExeUpdate('adm_siteviews', $ArrSiteViews, "WHERE siteviews_date = :date", "date={$this->Date}");

        $this->Traffic = null;
    }

    //Obtém dados da tabele
    //adm_siteviews
    private function getTraffic() {
        $readSiteViews = new Read;
        $readSiteViews->ExeRead('adm_siteviews', "WHERE siteviews_date = :date", "date={$this->Date}");
    ...
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('adm_sitevieadm_agent', "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResult()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('adm_sitevieadm_agent', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResult()[0]['agent_views'] + 1];
            $updateAgent = new Update;
            $updateAgent->ExeUpdate('adm_sitevieadm_agent', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
    ...
    private function setUsuario() {
        ...
        $userCreate->ExeCreate('adm_sitevieadm_online', $sesOnline);
    }
    ...
    private function UsuarioUpdate() {
        ...
        $userUpdate->ExeUpdate('adm_sitevieadm_online', $ArrOnlone, "WHERE online_session = :ses", "ses={$_SESSION['useronline']['online_session']}");

        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('adm_sitevieadm_online', 'WHERE online_session = :onses', "onses={$_SESSION['useronline']['online_session']}");
