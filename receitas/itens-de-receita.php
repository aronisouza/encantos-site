<?php
  ...
  if (isset($_GET['nomereceita'])):
    ...
    if(isset($_GET['pesquisa'])):
      ...
      if($pesquisa->getRowCount()):
        header("Location: itens-de-receita.php?idreceita={$pesquisa->getResult()[0]['id']}&nomereceita={$pesquisa->getResult()[0]['nome']}&rendimento={$pesquisa->getResult()[0]['rendimento']}&tipo={$pesquisa->getResult()[0]['tipo']}");
      else:
        // se nota não existe
      ...
      if(!isset($_POST['adicionar'])):
        $pega = new Read;
        $pega->ExeRead('receitas_itens', 'WHERE id_receita=:idreceita ORDER BY id DESC', "idreceita={$_GET['idreceita']}");
          ...
          for($i = 0 ; $i < $pega->getRowCount(); $i++):
            ...
            $pesquisaRetorno = $pesquisaRetorno ."
              <tr>
                ...
                <td>{$ft}</td>
                <td>
                  <a class=\"btn btn-danger botaoW\" href=\"{$URL_ATUAL}&apagar={$pega->getResult()[$i]['id']}\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"DELETAR => Ao clicar ele será deletado automaticamente... Perdeu!!!\"></a>
                </td>
                <td>
                  <a class=\"btn btn-info botaoW\" href=\"?editar={$pega->getResult()[$i]['id']}\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"EDITAR este item\"></a>
                </td>
              </tr>
            ";
          endfor;
    ...
    $jnSVG = 'exclamation-circle';
    $textoJanela = '
      Aqui você irá cadastrar os Itens de sua Receita.<br />
  ...
  if (isset($_POST['adicionar']) && !isset($_GET['editar'])):
    ...
    $cadItemNota->ExeCreate('receitas_itens', $formItem);
  ...
  if (isset($_GET['apagar'])):
    $ap_id = $_GET['apagar'];
    $del = new Delete;
    $del->ExeDelete('receitas_itens',"WHERE id=:id", "id={$ap_id}");
  ...
  if (isset($_GET['editar']) && !isset($_POST['adicionar'])):
    $ed_id = $_GET['editar'];
    $pega = new Read;
    $pega->ExeRead('receitas_itens', 'WHERE id=:idr', "idr={$ed_id}");
    $formEdita = [
      'id_receita'  => $pega->getResult()[0]['id_receita'],
      'materia'     => $pega->getResult()[0]['materia'],
      'volume'      => $pega->getResult()[0]['volume'],
      'preco_pago'  => $pega->getResult()[0]['preco_pago'],
      'utilizado'   => $pega->getResult()[0]['utilizado'],
      'frete'       => $pega->getResult()[0]['frete'],
      'tipo_volume' => $pega->getResult()[0]['tipo_volume']
    ];
    $corJanelaMensagem = 'bg-laranja';
    $txCor = 'tx-preto';
    $jnSVG = 'chat-dots';
    $textoJanela = "
      ...
      <p>Você esta Editando: <strong>{$formEdita['materia']}</strong></p>
    ";
  endif;
  ...
  if (isset($_GET['editar']) && isset($_POST['adicionar'])):
    ...
    $ulp = new Update;
    $ulp->ExeUpdate('receitas_itens', $formItem, "WHERE id=:id", "id={$ed_id}");
...
<?php echo baseMenu();?>
  ...
  <div class="navbar-collapse offcanvas-collapse" id="navbarsExampleDefault">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active"><a class="nav-link">Itens de uma Receita</a></li>
...
<main role="main" class="container">
  ...
  <div class="alert <?php echo $corJanelaMensagem;?> mt-3 shadow-sm" role="alert">
    <div class="d-flex">
      ...
      <div class="p-2 w-100">
        <h5 class="alert-heading <?php echo $txCor;?>">Cadastro de Itens de uma Receita</h5>
        <hr>
  ...
  <div class="my-3 p-3 bg-white rounded shadow-sm">
    ...
    <h6 class="border-bottom border-gray pb-2 mb-0">Adicionar Itens a uma Receita</h6>
    
    <form class="mt-4" id="formid" method="post" enctype='multipart/form-data'>

      <input type="hidden" id="id_receita" name="id_receita" value="<?php echo $formEdita['id_receita'];?>">

      <div class="form-row">
      
        <div class="form-group col-md-6">
          <label for="materia">Materia Prima</label>
          <input type="text" class="form-control" id="materia" name="materia" value="<?php echo $formEdita['materia'];?>">
        </div>
        <div class="form-group col-md-3">
          <label for="volume">Volume ( gr, ml, cm )</label>
          <input type="text" class="form-control" id="volume" name="volume" value="<?php echo $formEdita['volume'];?>">
        </div>
        <div class="form-group col-md-3">
          <label for="preco_pago">Preço Pago Nota</label>
          <input type="text" class="form-control" id="preco_pago" name="preco_pago" value="<?php echo $formEdita['preco_pago'];?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-3">
          <label for="frete">Frete ( 10% padrão )</label>
          <input type="text" class="form-control" id="frete" name="frete" value="<?php echo $formEdita['frete'];?>">
        </div>
        <div class="form-group col-md-3">
          <label for="utilizado">Quantidade Utilizada</label>
          <input type="text" class="form-control" id="utilizado" name="utilizado" value="<?php echo $formEdita['utilizado'];?>">
        </div>
        ...
        <div class="form-group mt-96 ml-2">
          <button class="btn btn-outline-info" type="submit" name="adicionar">Add Item a Receita</button>
        </div>
      </div>
